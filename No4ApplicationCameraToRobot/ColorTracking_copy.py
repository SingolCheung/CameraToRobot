...
import cv2
import time
sys.path.append('D:\\Informations\\VScode\\CameraToRobot\\No2ShowCameraHasBeenCalibrated\\')
import CameraHasBeenCalibrated
import threading
...
def run(img):
    ...
    if not start_pick_up:
        ...
        if area_max > 2500:  # 有找到最大面积
            ...
            #print("rect=",rect)
            #print("rect[0][0]=",rect[0][0])      #中心点坐标
            #print("rect[0][1]=",rect[0][1])      #中心点坐标
            #print("rect[1][0]=",rect[1][0])      #宽or高
            #print("rect[1]10]=",rect[1][1])      #宽or高
            #print("rect[2]=",rect[2])            #角度
            # cv2.boxPoints(rect)获取矩形四个顶点的坐标，浮点数;
            #  np.int0取整
            ...
            if action_finish:
                if distance < 0.3:
                    ...
                    if time.time() - t1 > 1:
                        ...
                        #print("rect[2] angle=",rotation_angle)
                        start_count_t1 = True
                        world_X, world_Y = np.mean(np.array(center_list).reshape(count, 2), axis=0)
                        count = 0
                        center_list = []
                        print("像素坐标last x={0},last y={1}".format(img_centerx, img_centery))
                        #print("现实坐标last x={0},last y={1}".format(world_x,world_y))
                        #start_pick_up = True
                else:
                    t1 = time.time()
                    start_count_t1 = True
...
if __name__ == '__main__':
    ...
    start()
    __target_color = ('red', )
    my_camera = CameraHasBeenCalibrated.Camera()
